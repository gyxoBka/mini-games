...
const saveFiles = () => {
    return src(distPath + '**/*.{css,js,svg,png,jpg,jpeg,webp,gif,eot,woff, woff2}', {base: distPath})
        .pipe(rev())
        .pipe(dest(distPath))
        .pipe(revDel())
...
const watching = () => {
    ...
    watch([path.pathWatch.svg], svgSprites);
...
const buildProject = series(cleanDist, parallel(styles, js, images, html, svgSprites, fonts));
